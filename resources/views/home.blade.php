...
<div class="bg-white w-full overflow-hidden">
    ...
    <section class="relative h-screen w-full overflow-hidden">
        ...
        {{-- Overlay Escuro (Mantido aqui para leitura do texto, mas suave) --}}
        ...
        <div class="container mx-auto px-6 relative z-10 h-full flex flex-col justify-center">
            <div class="max-w-5xl text-white">
                ...
                {{-- MUDANÇA 1: BUSCADOR NA HOME --}}
                <div class="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-sm max-w-4xl" data-aos="fade-up" data-aos-delay="400">
                    <form action="{{ route('properties.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        
                        {{-- Localização --}}
                        <div class="md:col-span-1">
                            <label class="block text-xs font-bold text-white uppercase tracking-wider mb-2">Localização</label>
                            <input type="text" name="location" placeholder="Cidade ou zona..." 
                                   class="w-full bg-white/90 border-0 text-gray-900 placeholder-gray-500 px-4 py-3 focus:ring-2 focus:ring-brand-premium rounded-sm">
                        </div>

                        {{-- Tipo --}}
                        <div>
                            <label class="block text-xs font-bold text-white uppercase tracking-wider mb-2">Tipo</label>
                            <select name="type" class="w-full bg-white/90 border-0 text-gray-900 px-4 py-3 focus:ring-2 focus:ring-brand-premium rounded-sm">
                        ...
                        {{-- Preço Máximo --}}
                        <div>
                            <label class="block text-xs font-bold text-white uppercase tracking-wider mb-2">Orçamento Máx.</label>
                        ...
                        {{-- Botão Buscar --}}
                        <div>
                            <button type="submit" class="w-full bg-brand-premium hover:bg-white text-brand-primary font-bold uppercase tracking-widest py-3 transition-colors duration-300">
                                Buscar
                            </button>
                        </div>
                    </form>
    ...
    {{-- 3. METODOLOGIA (Movido para cima na reorganização lógica, já que feedback desceu) --}}
    ...
    {{-- 6. PRIVATE OFFICE (OFF MARKET) --}}
    {{-- MUDANÇA 2: IMAGEM LIMPA (Removido o overlay bg-brand-primary/90) --}}
    <section id="contact" class="py-32 relative overflow-hidden">
        ...
        {{-- Overlay leve apenas para legibilidade caso a foto seja muito clara, mas mantendo o aspecto "limpo" --}}
        ...
        <div class="container mx-auto px-6 relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-left text-white drop-shadow-lg">
                    <h2 class="font-display text-5xl md:text-6xl mb-6">Private <span class="italic text-brand-premium">Office.</span></h2>
                    <p class="font-light text-lg md:text-xl leading-relaxed mb-8">
                ...
                {{-- Formulário --}}
                <div class="bg-black/40 backdrop-blur-md p-8 border border-white/10 rounded-sm">
                    <form action="{{ route('contact.send') }}" method="POST" class="space-y-6">
                        @csrf
                        <div class="space-y-1">
                            <label class="text-xs text-brand-premium uppercase tracking-widest ml-1">Nome</label>
                            <input type="text" name="name" required class="w-full bg-transparent border-b border-white/40 py-3 text-white focus:outline-none focus:border-brand-premium transition-colors placeholder-white/30" placeholder="Seu nome completo">
                        </div>
                        <div class="space-y-1">
                            <label class="text-xs text-brand-premium uppercase tracking-widest ml-1">Email</label>
                            <input type="email" name="email" required class="w-full bg-transparent border-b border-white/40 py-3 text-white focus:outline-none focus:border-brand-premium transition-colors placeholder-white/30" placeholder="seu@email.com">
                        </div>
                        <div class="space-y-1">
                            <label class="text-xs text-brand-premium uppercase tracking-widest ml-1">Telefone</label>
                            <input type="tel" name="phone" class="w-full bg-transparent border-b border-white/40 py-3 text-white focus:outline-none focus:border-brand-premium transition-colors placeholder-white/30" placeholder="+351 ...">
                        </div>
    ...
    {{-- 7. PENÚLTIMA DOBRA: FEEDBACK --}}
    {{-- MUDANÇA 3: Movi esta seção para cá --}}
    ...
    {{-- 8. ÚLTIMA DOBRA: PERGUNTAS FREQUENTES (FAQ) --}}
    {{-- MUDANÇA 3: Movi esta seção para o final --}}
